...
3. 创建游戏环境，需要抽象封装，支持简易版军棋游戏（如二国军棋、更小的棋盘、更少棋子等），四国军棋，明棋、暗棋的训练。支持gui交互
4. 训练明棋ai：
...
6. 视训练情况，考虑推广，获取投资。加强训练。出售产品等。

### 当前阶段目标
在 Othello（翻转棋）上用 AlphaZero 跑通完整训练流程，熟悉 OpenSpiel 的：
- 游戏注册与 State/Game API
- PyTorch/JAX 深度 RL 训练管线
- MCTS + 神经网络自博弈
- 模型评估与 UI 对战

选择 Othello 而非 TicTacToe 的原因：TicTacToe 太简单（5478 状态，Q-table 即可解决），无法体现神经网络泛化的必要性。Othello 状态空间 ~10^28，真正需要深度学习。
