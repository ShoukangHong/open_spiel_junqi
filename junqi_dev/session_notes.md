...
3. 进行中，需要注意中国象棋有与军棋类似的一些特点，要在中国象棋的训练中解决这些问题：
 - 对局长度很长，game有重复局面和棋的限制，当前使用了重复惩罚和最大未吃子步数进行限制。
 - 容易出现消极比赛导致和棋的局面，模型停止进步，当前使用了和棋惩罚进行限制。
 - action space复杂，不像围棋和othello一样action很有限，当前使用 90 * 90 的输出头处理。
 - 棋子类型多样，通道数多。 
最新进展：以上象棋问题基本解决，目前在训练中。已经有概率战胜高级电脑（baseline),估计棋力在业余顶尖水平。
